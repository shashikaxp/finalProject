...
?>
		  ...
		  $result2 = mysql_query($sql);	
		   ...
		   while ($row = mysql_fetch_array($result2)) {						  
		   	...
		   	foreach($id as $val){
				$data = mysql_query("SELECT sum(qty_reserve),sum(max_adult),sum(max_child) FROM roominventory WHERE room_id = '$val' AND  status != 'Cancelled' AND (arrival BETWEEN '$start' AND '$end' OR departure BETWEEN '$start' AND '$end')  ; ");	
